...
int _goboringcrypto_EVP_CIPHER_CTX_seal(
	...
	switch(key_size) {
		case 128:
			if (!_goboringcrypto_EVP_EncryptInit_ex(ctx, _goboringcrypto_internal_EVP_aes_128_gcm(), NULL, NULL, NULL)) {
				goto err;
			}
			break;
		case 256:
			if (!_goboringcrypto_EVP_EncryptInit_ex(ctx, _goboringcrypto_internal_EVP_aes_256_gcm(), NULL, NULL, NULL)) {
				goto err;
			}
			break;
		default:
	...
	if (!_goboringcrypto_internal_EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, 12, 0)) {
		goto err;
    }
    if (!_goboringcrypto_internal_EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IV_FIXED, -1, iv)) {
		goto err;
    }
    if (!_goboringcrypto_internal_EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_IV_GEN, 0, iv)) {
		goto err;
    }	
...
int _goboringcrypto_EVP_CIPHER_CTX_open(
	...
	switch(key_size) {
		case 128:
			if (!_goboringcrypto_internal_EVP_DecryptInit_ex(ctx, _goboringcrypto_internal_EVP_aes_128_gcm(), NULL, NULL, NULL)) {
				goto err;
			}
			break;
		case 256:
			if (!_goboringcrypto_internal_EVP_DecryptInit_ex(ctx, _goboringcrypto_internal_EVP_aes_256_gcm(), NULL, NULL, NULL)) {
				goto err;
			}
			break;
	}
	...
	if(!_goboringcrypto_internal_EVP_DecryptInit_ex(ctx, NULL, NULL, key, iv)) {
		goto err;
	}
	...
	*plaintext_len = len;
	...
	ret = _goboringcrypto_internal_EVP_DecryptFinal_ex(ctx, plaintext + len, &len);
