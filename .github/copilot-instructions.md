...
Before making any changes, ensure the submodule is initialized and patches are applied:

```bash
git go-patch apply
```

This applies all patches in `patches/` on top of the upstream Go source in `go/`.

### Step 2: Make changes in the `go/` submodule

Edit files directly inside `go/src/`. The key crypto backend files live at:

- `go/src/crypto/internal/backend/backend_linux.go` — OpenSSL shims
- `go/src/crypto/internal/backend/backend_windows.go` — CNG shims
- `go/src/crypto/internal/backend/backend_darwin.go` — CryptoKit shims
- `go/src/crypto/internal/backend/nobackend.go` — panic stubs (no backend)
- `go/src/crypto/internal/backend/common.go` — shared utilities
- `go/src/crypto/<algorithm>/` — algorithm-level integration (e.g., `ecdsa/boring.go`)

### Step 3: Build and test

Rebuild the toolchain if you changed anything under `go/src/cmd`:

```bash
cd go/src && ./make.bash
```

For standard library changes only, no rebuild is needed. Run tests directly:

```bash
cd go/src && go test ./crypto/<package>/...
```

### Step 4: Commit changes in the submodule

Commit your changes inside the `go/` submodule. The commit message becomes the patch description:

```bash
cd go
git add . --all
git commit -m "Add X algorithm to crypto backend"
```

### Step 5: Squash into the correct existing patch

Crypto backend changes must be squashed into the **existing** commit that corresponds to `0002-Add-crypto-backends.patch`. Do not create a new commit that sits after the existing patches — it will produce a new patch file instead of updating patch 0002.
...
In the interactive rebase editor, move your new commit directly below the `Add crypto backends` commit and change its action from `pick` to `fixup` (or `squash` if you want to edit the message).

### Step 6: Extract patches

After the rebase is complete, extract the updated patches:
...
This regenerates `patches/0002-Add-crypto-backends.patch` (and any other changed patches) from the submodule's Git history.

### Step 7: Verify and commit

Verify the submodule is clean and the patches are correct:

```bash
# Check that re-applying produces the same result
git go-patch apply
```

Then restore the submodule and commit only the patch file changes:

```bash
git submodule update --init --recursive --checkout
git add patches/
git commit -m "Wire X into the crypto backend"
```

**Important**: Do not commit the `go` submodule pointer change. Only commit files in `patches/`.
