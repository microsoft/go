	...
	for _, item := range section.Items {
		...
		for _, platform := range SupportedPlatforms {
			...
			default:
				...
				panic(fmt.Sprintf("unexpected status %v", status.Supported))
			}
	...
	for i := range section.Items {
		...
		for _, p := range SupportedPlatforms {
			...
			case Supported, NotSupported, Warn, N_A:
				// ok
			default:
				return fmt.Errorf("invalid supported status %v for item %q on platform %q", status.Supported, item.Name, p)
			}
		}
