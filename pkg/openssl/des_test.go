...
func newDESCipher(key []byte) cipher.Block {
	c, err := openssl.NewDESCipher(key)
	if err != nil {
		panic("NewDESCipher failed: " + err.Error())
	}
	return c
}
...
func TestDESEncryptTripleDES(t *testing.T) {
	...
	for i, tt := range encryptTripleDESTests {
		c, _ := openssl.NewTripleDESCipher(tt.key)
		out := make([]byte, len(tt.in))
		c.Encrypt(out, tt.in)
...
func TestDESVariablePlaintextKnownAnswer(t *testing.T) {
	...
	for i, tt := range tableA1Tests {
		c, _ := openssl.NewTripleDESCipher(tableA1Key)
...
func TestDESCBCBlobEncryptBasicBlockEncryption(t *testing.T) {
	...
	block, err := openssl.NewDESCipher(key)
	if err != nil {
...
func BenchmarkEncrypt(b *testing.B) {
	...
	c, err := openssl.NewDESCipher(tt.key)
	if err != nil {
...
func BenchmarkTDESEncrypt(b *testing.B) {
	if !openssl.SupportsTripleDESCipher() {
		b.Skip("3DES is not supported")
	}
	tt := encryptTripleDESTests[0]
	c, err := openssl.NewTripleDESCipher(tt.key)
	if err != nil {
		b.Fatal("NewCipher:", err)
	}
	out := make([]byte, len(tt.in))
